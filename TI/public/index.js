...
class AppModel {
  ...
  static async addTasklist(tasklistName, maxPlayers) {
    const result = await fetch(
      'http://localhost:4321/tasklists',
      {
        ...
        body: JSON.stringify({ tasklistName, maxPlayers })
      }
    );
  ...
  static async editTaskList({
    ...
    const result = await fetch(
        `http://localhost:4321/tasklists/${tasklistId}`,
        {
          ...
          body: JSON.stringify({ newTasklistName })
        }
    );
...
class App {
  ...
  onEscapeKeydown = ({ key }) => {
    if (key === 'Escape') {
      ...
      const maxInput = document.getElementById('add-tasklist-maxplayer-input');
      const confirmButton = document.getElementById('tm-tasklist-confirm-add');
  ...
  onInputKeydown = async () => {
    ...
    if (input.value && maxInput.value) {
        console.log(input.value, maxInput.value);
        await AppModel.addTasklist(input.value, maxInput.value);

        this.tasklists.push(
          new Tasklist({
            tlName: input.value,
            tlID: `TL${this.tasklists.length}`,
            maxPlayers: maxInput.value,
  ...
  moveTask = async ({ taskID, direction }) => {
    ...
    console.log(this.tasklists[targetTlIndex].tasks.length)

    if(this.tasklists[targetTlIndex].tasks.length == this.tasklists[targetTlIndex].maxPlayers)
    {
      alert('Нельзя переместить участника, так как в данной команде достигнуто максимальное количество игроков!');
      return;
    }
  ...
  async init() {
    ...
    tasklists.forEach(({ tasklistName, maxPlayers, tasks }) => {
    ...
    document.getElementById('tm-tasklist-add-tasklist')
      .addEventListener(
        ...
        (event) => {
          ...
          maxInput.style.display = 'inherit';
          confirmButton.style.display = 'inherit';
    ...
    document.getElementById('tm-tasklist-confirm-add')
        .addEventListener('click', this.onInputKeydown);
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    if (this.maxPlayers == this.tasks.length)
    {
      alert('Достигнуто максимальное количество участников в команде!');
      return;
    }

    const newTaskName = prompt('Введите ФИО участника:');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое ФИО участника', oldTaskName);
  ...
  onEditList = async () => {
    const newTasklistName = prompt('Введите новое название команды');

    if (!newTasklistName) {
      alert('Введите корректное название команды!')
      return;
    }
    ...
    try {
      ...
      this.tlName = newTasklistName;

      document.querySelector(`#${this.tlID} header .tm-tasklist-title`)
          .innerHTML = newTasklistName;
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Участник '${taskName}' будет удален из команды. Продолжить?`)) return;
  ...
  renderTask = ({ taskID, taskName, maxPlayers }) => {
  ...
  render() {
    ...
    listName.classList.add('tm-tasklist-title');
    listName.innerHTML = this.tlName;
    header.appendChild(listName);
    tasklist.appendChild(header);

    const playersSpan = document.createElement('div');
    playersSpan.innerHTML = this.maxPlayers;
    playersSpan.classList.add('tm-tasklist-task-text');
    playersSpan.style.marginLeft = '20px';
    ...
    editButton.type = 'button';
    editButton.classList.add('tm-tasklist-add-task');
    editButton.innerHTML = 'Редактировать';
    editButton.addEventListener('click', this.onEditList);
