...
app.post('/tasklists', async (request, response) => {
  const { tasklistName, maxPlayers } = request.body;
  tasklists.push({
    tasklistName,
    maxPlayers,
    tasks: [],
...
app.put('/tasklists/:tasklistId', async (req, res) => {
  const name = req.body.newTasklistName;
  ...
  tasklists[tasklistId].tasklistName = name;
  ...
  res
      ...
      .status(200)
      .json({
        info: `Tasklist '${tasklistId} was successfully updated!'`
      });
})
...
app.listen(port, hostname, async (err) => {
  ...
  tasklistsFromFile.forEach(({ tasklistName, maxPlayers, tasks }) => {
    tasklists.push({
      tasklistName,
      maxPlayers,
      tasks: [...tasks]
